  ...
  `
    ...
    document.querySelectorAll("accordion").forEach((accordion) => {
      ...
      accordion.querySelectorAll("accordionItem").forEach((item) => {
        ...
        item.style.borderBottom = `1px solid ${colorChanger(
          0.8,
          colorScheme()
        )}`;
      });
      accordion.querySelectorAll("accordionButton").forEach((btn) => {
        ...
        btn.addEventListener("click", function () {
          ...
          if (panel.style.height == panel.scrollHeight + "px") {
            ...
            this.querySelector("accordionIcon").style.transform =
              "rotate(45deg)";
          } else {
      ...
      accordion.querySelectorAll("accordionPanel").forEach((panel) => {
        ...
        panel.innerHTML = `<div class="inner" style="margin: ${
          size() / 2
        }px 0">${panel.innerHTML}</div>`;
      });
    ...
    document.querySelectorAll("radio").forEach((radio) => {
      ...
      newRadio.addEventListener("click", function () {
        ...
        this.querySelector("div").style.border = `${
          size() / 4
        }px solid ${color()}`;
      });
  ...
  `
    ...
    `
      ...
      if (autocompleteQuery != null) {
        var suggestionsList = field
          .getAttribute("autoCompleteQuery")
          .split(/,\s+/);
